 ...
 * │ TIM3    │ CH3/CH4      │ PB0/PB1     │ 1kHz    │ 上部电机(L298N)│
 * └─────────┴──────────────┴─────────────┴─────────┴────────────────┘
 *
 * 函数接口：
 * - Servo_SetAngle(angle)       : 设置舵机角度 0°~180°
 * - Motor_SetSpeedLeft(speed)   : 控制左侧电机 -127~+127
 * - Motor_SetSpeedRight(speed)  : 控制右侧电机 -127~+127
 * - Motor_SetSpeedTop(speed)    : 控制上部电机 -127~+127
 *
 * 电机控制逻辑（L298N H桥驱动）：
...
void HAL_TIM_MspPostInit(TIM_HandleTypeDef* timHandle)
{
  ...
  /* USER CODE END TIM2_MspPostInit 0 */
    ...
    PA2     ------> TIM2_CH3
    */
    GPIO_InitStruct.Pin = L298N_L_IN1_Pin|L298N_L_IN2_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
