...
void MX_FREERTOS_Init(void)
{
    ...
    motorQueue01Handle = osMessageQueueNew(3, 12, &motorQueue01_attributes);
 ...
 *
 *         【底部电机】双摇杆混合控制 + 肩键辅助
 *         - motorQueue01: 左摇杆(0.7权重) + 右摇杆(0.3权重) + L1/L2(左轮) + R1/R2(右轮)
 *           * 左摇杆: leftY/leftX × 0.7（主控制）
 *           * 右摇杆: rightY/rightX × 0.3（辅助控制）
...
void StartMotorTask01(void *argument)
{
    ...
    const int16_t BUTTON_SPEED        = 100;  // 肩键控制速度（0-127范围）
        ...
        if (osMessageQueueGet(motorQueue01Handle, &motorData, NULL, 0) == osOK) {
            ...
            if (buttons & 0x10) { // 圆圈键按下（bit4）
                ...
                if (joystickActive) {
                    ...
                    // 加权混合（0.7 + 0.3）
                    leftSpeed_f  = L_left * 0.7f + R_left * 0.3f;
                    rightSpeed_f = L_right * 0.7f + R_right * 0.3f;
 ...
 *         - 恢复正常 → 电机速度恢复到100%
 *
 *         注意：坦克式控制下，左右电机速度由rightY±rightX计算得出，
 *               碰撞时通过调整leftMotorScale/rightMotorScale缩放比例实现避障
 *
 *         平滑过渡算法：
